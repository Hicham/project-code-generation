...
//    List<Transaction> findBySourceIBANAndTimestampBetween(String sourceIBAN, Long startDate, Long endDate);
    ...
    @Query("SELECT SUM(t.amount) FROM Transaction t WHERE t.sourceIBAN = :sourceIBAN AND t.timestamp BETWEEN :startDate AND :endDate")
    Double findSumBySourceIBANAndTimestampBetween(@Param("sourceIBAN") String sourceIBAN, @Param("startDate") Long startDate, @Param("endDate") Long endDate);
}
